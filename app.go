...
func (a *App) WriteAll(outputDir *string) ([]string, error) {
	var written []string
	...
	for _, p := range a.Packages {
		for _, t := range p.Types {
			for _, tw := range a.TypeWriters {
				var b bytes.Buffer
				n, err := write(&b, a, p, t, tw, outputDir)
	...
	for f, b := range buffers {
		...
		path := filepath.Join(fileDirectory, f)
...
func pathSubstring(s string) string {
	index := 0
	for i, v := range s {
		if string(v) == "/" {
			index = i
...
func getLastFolder(path string) string {
	path = pathSubstring(path)
	return path
}

var twoLines = bytes.Repeat([]byte{'\n'}, 2)

func write(w *bytes.Buffer, a *App, p *Package, t Type, tw Interface, outputDir *string) (n int, err error) {
...
// Directive: %s on %s`
	...
	pkg := fmt.Sprintf("package %s", packageName)
	w.Write([]byte(pkg))
	w.Write(twoLines)
